...
if (class_exists('Predis_Client')) {
	...
	class RedisApi extends Predis_Client
	{
	    /**
		 * Redis namespace
		 * @var string
		...
		{
			$params = array(
				'host' => $host, 'port' => $port, 'connection_timeout' => $timeout
			);
			if ($password !== null) {
			...
			$this->connect();
			$this->prefix(self::$defaultNamespace);
		}
		...
		public function prefix($namespace)
		{
            ...
            //New versions of Predis have Command Processor
//			$prefixer = new Predis\Command\Processor\KeyPrefixProcessor(self::$defaultNamespace);
...
//			 renamenx
		 ...
		 * @return mixed Return value from PRedis::call() based on the command.
		 */
		public function __call($name, $args) {
			...
			try {
				return parent::__call(strtolower($name), $args[1]);
			}
...
class Resque_Redis extends redisApi {
	...
	{
		if(is_subclass_of($this, 'Redisent')){
